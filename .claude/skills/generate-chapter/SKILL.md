...
Show total estimated cost. Ask for explicit approval before generating.
...
The tool generates frame A, then feeds frame A's output as a visual anchor into frame B — both in the same conversation session. Saves `{id}-frame-a-{timestamp}.png` and `{id}-frame-b-{timestamp}.png`.
